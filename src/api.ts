...
const ddbSavePost = async (post: SavePostProps) => {
  ...
  const contentString = JSON.stringify(post.description);
...
const ddbGetPost = async (postId: string) => {
  ...
  const resp = await API.get("MyApi", 'posts', query);
...
const ddbGetPosts = async () => {
  ...
  const resp = await API.get("MyApi", 'posts', query);
  console.log(`data from API Gateway: ${JSON.stringify(resp, null, 2)}`);
  // @ts-ignore
